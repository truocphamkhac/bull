...
var LOCK_RENEW_TIME = process.env.LOCK_RENEW_TIME || 5000; // 5 seconds is the renew time.
var CLIENT_CLOSE_TIMEOUT_MS = 5000;
var POLLING_INTERVAL = 5000;

var Queue = function Queue(name, redisPort, redisHost, redisOptions){
  if(!(this instanceof Queue)){
    console.log('[bull]', 'Create a new instance');
    return new Queue(name, redisPort, redisHost, redisOptions);
  }

  console.log('[bull]', 'Re-use an existed instance');
  ...
  this.eclient.on('message', function(channel, message){
    if(channel === _this.toKey('delayed')){
      console.log('[bull]', 'Hanle delay', message);
  ...
  console.log('[bull]', 'Init delay timestamp', this.delayedTimestamp);
...
Queue.prototype.updateDelayTimer = function(newDelayedTimestamp){
  ...
  console.log('[bull]', 'updateDelayTimer', 'newDelayedTimestamp:', newDelayedTimestamp);
  console.log('[bull]', 'updateDelayTimer', '_this.delayedTimestamp:', _this.delayedTimestamp);
  ...
  if(!this.closed){
    process.nextTick(function(){
      (_this.paused || Promise.resolve())
        ...
        .finally(function () {
          if (_this.isLowMemoryExit) {
            console.log('[bull]', 'LowMemoryExit');
            process.exit(0);
          }
        });
    });
  ...
  return lockRenewer().then(function(locked){
    if(locked){
      ...
      if (Memory.isLowMemory()) {
        console.log('[bull]', 'Memory.isLowMemory', 2 * 60 * 1000);
        memoryDelayTime = 2 * 60 * 1000;
        _this.isLowMemoryExit = true;
      } else {
        memoryDelayTime = 0;
        _this.isLowMemoryExit = false;
      }

      return jobPromise
        .delay(memoryDelayTime)
        .then(handleCompleted, handleFailed)
        .finally(function(){
          console.log('[bull]', 'clear lockRenewId', lockRenewId);
